...
class CuboidPLModule(pl.LightningModule):

    def __init__(self,
                 total_num_steps: int,
                 oc_file: str = None,
                 save_dir: str = None):
        super(CuboidPLModule, self).__init__()
    ...
    @staticmethod
    def get_datamodule(dataset_oc, micro_batch_size: int = 1, num_workers: int = 2):
        ...
        train_path = os.path.expanduser(dataset_oc.get("tain_path", "/data1/h5data/train_data/train_data_2.h5"))
        val_path = os.path.expanduser(dataset_oc.get("val_path", "/data1/h5data/val_data/val_data_2.h5"))
        test_path = os.path.expanduser(dataset_oc.get("test_path", "/data1/h5data/test_data/test_data_2.h5"))
    ...
    def training_step(self, batch, batch_idx):
        ...
        if str(self.trainer.precision) == "16-mixed":
            y_hat = y_hat.float()
            y = y.float()
...
def main():
    ...
    if args.cfg is not None:
        ...
        dataset_oc = OmegaConf.to_object(CuboidPLModule.get_dataset_config())
        micro_batch_size = 1
