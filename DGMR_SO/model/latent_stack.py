...
class LBlock(snt.Module):
    ...
    def __init__(self, output_channels, input_channels, kernel_size=3, conv=layers.Conv2D, activation=tf.nn.relu):
        ...
        self._conv_residual = conv(output_channels=self._output_channels - self._input_channels, kernel_size=1)
...
class Attention(snt.Module):
    ...
    def __init__(self, num_channels, ratio_kq=8, ratio_v=8, conv=layers.Conv2D):
        ...
        self._conv1 = conv(output_channels=self._num_channels // self._ratio_kq, kernel_size=1, padding='VALID', use_bias=False)
        self._conv2 = conv(output_channels=self._num_channels // self._ratio_kq, kernel_size=1, padding='VALID', use_bias=False)
        self._conv3 = conv(output_channels=self._num_channels // self._ratio_v, kernel_size=1, padding='VALID', use_bias=False)
