...
class ConditioningStack(snt.Module):
    ...
    def __call__(self, inputs, is_training=True):
        ...
        h0 = batch_apply(functools.partial(tf.nn.space_to_depth, block_size=2), inputs)
        ...
        # Return a stack of conditioning representations of size 64x64x48, 32x32x96, 16x16x192 and 8x8x384.
...
class Sampler(snt.Module):
    ...
    def __call__(self, initial_states, resolution, is_training=True):
        ...
        return tf.cast(tf.stack(hs, axis=1), tf.float16)
...
class GBlock(snt.Module):
    ...
    def __init__(self, output_channels, sn_eps=0.0001):
        ...
        self._conv2_3x3 = layers.SNConv2D(output_channels, kernel_size=3, sn_eps=sn_eps)
        self._bn2 = layers.BatchNorm()
        self.conv_1x1 = layers.SNConv2D(output_channels, kernel_size=1, sn_eps=sn_eps)
    ...
    def __call__(self, inputs, is_training=True):
        ...
        # Two-layer residual connection, with batch normalization, nonlinearity and 3x3 spectrally normalized convolution in each layer.
...
class UpsampleGBlock(snt.Module):
    ...
    def __call__(self, inputs, is_training=True):
        ...
        # Two-layer residual connection, with batch normalization, nonlinearity and 3x3 spectrally normalized convolution in each layer, and x2 upsampling in the first layer.
        h = tf.nn.relu(self._bn1(inputs, is_training=is_training))
...
class ConvGRU(snt.Module):
    ...
    def __init__(self, kernel_size=3, sn_eps=0.0001, num_channels=768):
        ...
        self._sn_eps = sn_eps
        self._conv1 = layers.SNConv2D(num_channels, self._kernel_size, sn_eps=self._sn_eps)
        self._conv2 = layers.SNConv2D(num_channels, self._kernel_size, sn_eps=self._sn_eps)
        self._conv3 = layers.SNConv2D(num_channels, self._kernel_size, sn_eps=self._sn_eps)
