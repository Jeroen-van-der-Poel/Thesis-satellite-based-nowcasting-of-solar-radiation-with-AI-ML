...
class Discriminator(snt.Module):
    ...
    def __init__(self):
        ...
        # As the input is the whole sequence of the event (including conditioning frames), the spatial discriminator needs to pick only the t > T+0.
    ...
    def __call__(self, frames, is_training=True):
        ...
        # Prepare the frames for spatial discriminator: pick 8 random time steps out of 18 lead time steps, and downsample from 256x256 to 128x128.
        ...
        # Prepare the frames for temporal discriminator: choose the offset of a random crop of size 128x128 out of 256x256 and pick full sequence samples.
...
class DBlock(snt.Module):
    ...
    def __init__(self, output_channels, kernel_size=3, downsample=True,
        ...
        self._conv1 = conv(output_channels=self._output_channels, kernel_size=self._kernel_size)
        self._conv2 = conv(output_channels=self._output_channels, kernel_size=self._kernel_size)
        self._residual_conv = conv(output_channels=self._output_channels, kernel_size=1)
    ...
    def __call__(self, inputs, is_training=True):
        ...
        # The residual connection, make sure it has the same dimensionality with additional 1x1 convolution and downsampling if needed.
...
class SpatialDiscriminator(snt.Module):
    ...
    def __call__(self, frames, is_training=True):
        ...
        # Five residual D Blocks to halve the resolution of the image and double the number of channels.
...
class TemporalDiscriminator(snt.Module):
    ...
    def __call__(self, frames, is_training=True):
        ...
        # Two residual 3D Blocks to halve the resolution of the image, double the number of channels, and reduce the number of time steps.
        y = self._block1(frames, is_training=is_training)
        y = self._block2(y, is_training=is_training)
