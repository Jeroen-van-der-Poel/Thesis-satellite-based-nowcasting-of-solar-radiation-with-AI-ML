...
from EarthFormer.visualization.sevir.sevir_vis_seq_denorm import save_example_vis_results, save_comparison_vis_results_vertical
    ...
    for idx, batch in enumerate(tqdm(dataloader, desc=f"Evaluating {model_name}")):
        ...
        for t in range(T):
            try:
                ...
                sds_t = sds_cs_targets[:, t]
                if model_name == "DGMR-SO" or cropping:
                    baseline_crop = np.stack([
                        baseline[b, y:y+256, x:x+256]
                        for b, (y, x) in enumerate(zip(y_coords, x_coords))
                    ])
                    sds_cropped = np.stack([
                        sds_t[b, y:y+256, x:x+256]
                        for b, (y, x) in enumerate(zip(y_coords, x_coords))
                    ])
                    baseline_crop = baseline_crop * sds_cropped
...
if __name__ == "__main__":
    DGMR_CHECKPOINT_DIR = "../DGMR_SO/experiments/solar_nowcasting_v10/"
