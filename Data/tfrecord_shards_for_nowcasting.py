...
class Nowcasting_tfrecord():
    ...
    def write_images_to_tfr_long(self, cond,targ, date, prob=None):
        ...
        if self.first or self.current_shard_count >= self.max_samples:
            if self.writer:
                ...
                self.writer.close()
            # open a new file
            self.current_shard_name = self.out_dir / "{}.tfrecords".format(self.filename)
        ...
        if prob:
            out = self.parse_single_image_with_prob(
                cond=cond, targ=targ,  prob=prob, start_date=date)
        else:
            out = self.parse_single_image(
                cond=cond,targ=targ,start_date=date)
    ...
    def parse_single_image(self, cond, targ,  start_date):
